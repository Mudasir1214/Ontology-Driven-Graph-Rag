...
OUTPUT_FILE = f"{OUTPUT_DIR}/Benchmark_Results_Final_Ultimate.xlsx"
TEXT_DATA_FILE = "./input_data/text_data.json"

# ================= 0. Question Data (151 questions full version) =================
QUESTIONS_DATA = [
...
print("🔌 Connecting to DeepSeek and Neo4j...")
try:
    ...
    print(f"❌ Database connection failed! Please check password. Error: {e}")
    exit()
...
def local_retrieve_text(query):
    entries = get_text_data()
...
def local_retrieve_graph(query):
    """
    【V8 Ultimate - Nuclear-level brute-force retrieval】
    No longer guessing property names, directly iterate over all node keys to find matches.
    No matter if your property is named 'name', 'Name', 'title', 'Value', 'text', 'desc'...
    As long as the value contains keywords, it will be retrieved!
    """
    context = ""
    try:
        with driver.session() as session:
            # 1. Keyword extraction (keep original cleaning logic, very good)
            raw_keywords = str(query).lower().replace("?", "").replace("chapter", "").replace("section", "").split()
            stop_words = ["does", "the", "include", "any", "material", "specifications", "if", "not", "where", "are", "they", "defined", "what", "is", "in", "install", "requirement", "provide", "list", "value", "of", "for", "a", "an", "reference"]
            
            # Relax constraints: numbers or words with length >1 are valid
            valid_keywords = [k for k in raw_keywords if (k not in stop_words and len(k) > 1) or k.isdigit()]
            
            if not valid_keywords: valid_keywords = ["general"] 

            # 2. Nuclear-level Cypher query: ANY(k IN keys(n)...)
            # If any property value of node n contains keywords, it matches!
            # After finding core nodes, expand 1-2 hops to capture neighbors
            cypher_query = f"""
            WITH {json.dumps(valid_keywords)} as keywords
            MATCH (n)
            WHERE ANY(k IN keys(n) WHERE ANY(word IN keywords WHERE toLower(toString(n[k])) CONTAINS word))
            
            // After finding core nodes, find neighbors (undirected, 1-2 hops)
            OPTIONAL MATCH path = (n)-[*1..2]-(target)
            
            WITH path, n
            UNWIND nodes(path) as node
            RETURN DISTINCT 
                labels(node)[0] as Type,
                // Brute display: output all properties to avoid missing info
            ...
            for row in result:
                ...
                # Smart extraction of Name and Content, with fallback strategy
                name = props.get('name') or props.get('title') or props.get('id') or "Unnamed"
                
                # Try extracting meaningful text content
                content_candidates = []
                for k, v in props.items():
                    if k in ['name', 'title', 'id']: continue # skip name fields
                    v_str = str(v)
                    if len(v_str) > 1 and not v_str.startswith("{"): # simple filtering
                        content_candidates.append(f"{k}: {v_str}")
                
                content_str = " | ".join(content_candidates)
                if len(content_str) > 300: content_str = content_str[:300] + "..."
                
                lines.append(f"[{node_type}] {name} || {content_str}")
    ...
    return context if context else "No graph path found."
...
def run_benchmark():
    ...
    if os.path.exists(OUTPUT_FILE):
        try:
            ...
            print(f"❌ Error: Please close '{OUTPUT_FILE}' first!")
            return

    results = []
    print(f"🚀 Starting benchmark (V8 Ultimate)! Total {len(QUESTIONS_DATA)} questions.")
    ...
    for item in tqdm(QUESTIONS_DATA):
        ...
        try:
            ...
            row["模型回复：Standalone LLM"] = res.choices[0].message.content
        except: row["模型回复：Standalone LLM"] = "Error"

        # 2. Mode 2: Text RAG
        try:
            ctx = local_retrieve_text(prompt)
            if ctx:
                sys = f"Answer based on:\n{ctx}"
                res = client.chat.completions.create(model="deepseek-chat", messages=[{"role":"system","content":sys},{"role":"user","content":prompt}])
                row["模型回复：Text-based RAG"] = res.choices[0].message.content
            else:
                row["模型回复：Text-based RAG"] = "Not found in text documents."
        except: row["模型回复：Text-based RAG"] = "Error"

        # 3. Mode 3: Ontology RAG (V8 Ultimate Fix)
        try:
            ctx = local_retrieve_graph(prompt)
            if "No graph path" not in ctx and "Graph Error" not in ctx:
                sys = f"""You are an engineering expert. Answer based STRICTLY on the Knowledge Graph Context provided below.
                
                Context Format:
                [NodeType] NodeName || Property1: Value1 | Property2: Value2 ...
                
                Instructions:
                - The context contains raw database nodes.
                - Look for keywords in the properties to find the answer.
                - If asked for values (e.g. dimensions, limits), check the 'Value' or 'text' properties.
                - If asked about structure, look at which nodes appear together.
                
                Context:
                {ctx}"""
                res = client.chat.completions.create(model="deepseek-chat", messages=[{"role":"system","content":sys},{"role":"user","content":prompt}])
                row["模型回复：Ontology+LLM"] = res.choices[0].message.content
            else:
                row["模型回复：Ontology+LLM"] = "Graph path not found (Check keywords)."
        except: row["模型回复：Ontology+LLM"] = "Error"
    ...
    target_cols = ["问题编号", "大模型提示词", "模型回复：Standalone LLM", "模型回复：Text-based RAG", "模型回复：Ontology+LLM", "标准答案 (Ground Truth)", "注：模型回复准确用黄色背景高亮表示"]
    df_res = df_res[[c for c in target_cols if c in df_res.columns]]
    
    # Use openpyxl engine to avoid xlsxwriter version issues
    ...
    for r in range(2, ws.max_row + 1):
        for c in range(1, ws.max_column + 1):
            ...
            try:
                qid = int(ws.cell(row=r, column=1).value)
                if c == 5 and qid <= 100: cell.fill = yellow 
                if c == 5 and qid > 100: cell.fill = green
            except: pass
